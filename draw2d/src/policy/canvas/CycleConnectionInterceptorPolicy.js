 ...
 * @since 5.0.0
 */
draw2d.policy.canvas.CycleConnectionInterceptorPolicy = draw2d.policy.canvas.ConnectionInterceptorPolicy.extend({

    NAME : "draw2d.policy.canvas.CycleConnectionInterceptorPolicy",
    ...
    delegateDrop: function(draggedFigure, dropTarget)
    {
        ...
        // It is also not possible to create cycles in the application.
        // Reason: Cycles would result in update loops after an event.
        if ((draggedFigure instanceof draw2d.Port) && (dropTarget instanceof draw2d.Port)) {
            ...
            if (reactiveProcessor.reactiveGraph.createsCycle(
                    draggedFigure.getParent().getId(),
                    dropTarget.getParent().getId())) {
                return null;
            }
        }
